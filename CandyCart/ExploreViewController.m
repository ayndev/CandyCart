...
{
    //[[DataService instance] refreshProducts]; // Get product categories in browse
}
...
- (void)refreshControl:(ODRefreshControl *)refreshControl
{
    ...
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
        ...
        [[DataService instance] getRecentItems]; // Get product categories in browse
...
- (void)viewDidLoad
{
    ...
    NSArray *segmentedArray = [[NSArray alloc]initWithObjects:@"Recent",@"Random",@"Featured",nil];
    
    NSArray *ary = segmentedArray;
...
-(void)valueChanged
{
    ...
    NSString *type;
    @try{
        type = [[tempArray objectAtIndex:indexnumber] objectForKey:@"type"];
    ...
    for(int i=0;i<[allFeaturedArray count];i++)
    {
        ...
        if([[[productInfo objectForKey:@"general"] objectForKey:@"product_type"] isEqualToString:@"simple"] || [[[productInfo objectForKey:@"general"] objectForKey:@"product_type"] isEqualToString:@"external"])
        {
            ...
            [self fullImageBox:[[productInfo objectForKey:@"product_gallery"] objectForKey:@"featured_images"]
                  ...
                  regularPrice:[[[productInfo objectForKey:@"general"] objectForKey:@"pricing"] objectForKey:@"regular_price"]
                     salePrice:[[[productInfo objectForKey:@"general"] objectForKey:@"pricing"] objectForKey:@"sale_price"]
...
-(void)fullImageBox:(NSString*)url title:(NSString*)title currency:(NSString*)currency regularPrice:(NSString*)price salePrice:(NSString*)salePrice isOnSale:(BOOL)isOnSale productInfo:(NSDictionary*)productInfo notSimple:(BOOL)notSimple{
    ...
    PhotoBox *box = [PhotoBox fullImageBox:CGSizeMake(280,200) pictureURL:url title:title currency:currency regularPrice:price salePrice:salePrice isOnSale:isOnSale notSimple:notSimple];
