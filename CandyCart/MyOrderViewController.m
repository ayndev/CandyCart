...
-(void)filterAction{
    ...
    popover.border = YES;
    popover.contentSize = CGSizeMake(170,380);
...
-(void)setOrderView{
    ...
    for(int i=0;i<[lists count];i++)
    {
        ...
        [self order_box:[list objectForKey:@"orderID"] status:[list objectForKey:@"status"] orderDate:[list objectForKey:@"orderDate"] price:[NSString stringWithFormat:@"%@ %@", [[SettingDataClass instance] getCurrencySymbol],[list objectForKey:@"order_total"]] orderInfo:list];
    }
    }
